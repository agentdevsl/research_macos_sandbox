/**
 * Test SDK with default permissions as non-root user
 */
...
async function main() {
  console.log('=== SDK Test: Default Permissions as Non-Root ===\n');
  ...
  const id = generateSandboxId('defperm');
  ...
  // Create test script with DEFAULT permissions (no bypassPermissions)
  const testScript = `
...
async function main() {
  ...
  console.log('Starting query with DEFAULT permissions (not bypass)...');
  ...
  try {
    const q = query({
      ...
      options: {
        ...
        maxTurns: 1,
        cwd: '/workspace',
        // Note: NOT using bypassPermissions - using default mode
      }
    });
    let response = '';
...
ENDSCRIPT`]);
  ...
  console.log('\n=== Running SDK with default permissions ===');
