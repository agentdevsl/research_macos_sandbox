/**
 * Test SDK with Apple Container provider (macOS 26+ native containers)
 */
...
async function main() {
  console.log('=== SDK Test with Apple Container ===\n');
  console.log('Token:', authToken.slice(0, 25) + '...');

  const provider = new AppleContainerProvider();
  const available = await provider.isAvailable();
  if (!available) {
    console.error('Apple Container CLI not available (requires macOS 26+)');
    process.exit(1);
  }
  ...
  const id = generateSandboxId('apple');
  console.log('\nCreating sandbox:', id);
  ...
  const sandbox = await provider.create({
    id,
    image: 'node:22-slim',  // Debian-based image with Node.js
    mountPath,
    memoryMib: 2048,
    cpus: 2,
  ...
  // Install SDK (Node.js already included in image)
  console.log('\nInstalling SDK...');
