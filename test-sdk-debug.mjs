/**
 * Test SDK with DEBUG to see exact command
 */
...
async function test() {
  ...
  const id = generateSandboxId('sdk-debug');
  ...
  const sandbox = await provider.create({
    ...
    env: {
      ...
      CI: 'true',
      TERM: 'dumb',
      DEBUG_CLAUDE_AGENT_SDK: '1',  // Enable SDK debug output
    },
  });
...
async function main() {
  console.log('Starting SDK test with DEBUG...');
  ...
  try {
    const q = query({
      ...
      options: {
        ...
        stderr: (msg) => console.log('[CLI STDERR]', msg),
      }
    });

    let response = '';
    for await (const msg of q) {
      console.log('Message:', JSON.stringify(msg).slice(0, 300));
...
ENDSCRIPT`]);
  ...
  console.log('\n=== SDK Test with DEBUG ===');
  result = await sandbox.exec('sh', ['-c', 'cd /workspace && DEBUG_CLAUDE_AGENT_SDK=1 timeout 120 node test.js 2>&1']);
