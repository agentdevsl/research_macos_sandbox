/**
 * Test SDK with default permission mode (running as root)
 */
...
async function test() {
  ...
  const id = generateSandboxId('sdk-defperms');
  ...
  // Test SDK with default permission mode
  const testScript = `
const { query } = require('@anthropic-ai/claude-agent-sdk');

async function main() {
  console.log('Starting SDK test (default permissions)...');
  const start = Date.now();

  try {
    const q = query({
      prompt: 'Reply with exactly one word: WORKING. Do not use any tools.',
      options: {
        model: 'claude-sonnet-4-20250514',
        maxTurns: 1,
        cwd: '/workspace',
        // NO permissionMode - use defaults
