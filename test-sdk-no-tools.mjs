/**
 * Test SDK without tools option to avoid --tools "" issue
 */
...
async function test() {
  ...
  const id = generateSandboxId('sdk-notools');
  ...
  // Test SDK WITHOUT tools option
  const testScript = `
const { query } = require('@anthropic-ai/claude-agent-sdk');

async function main() {
  console.log('Starting SDK test WITHOUT tools option...');
  ...
  try {
    const q = query({
      ...
      options: {
        ...
        // NO tools option - let SDK use defaults
      }
    });
